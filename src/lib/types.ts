...
export interface User {
  ...
  passHash: string; salt: string; suspended: boolean; onboarded: boolean;
  purpose: string | null; prefs: UserPrefs; createdAt: string; updatedAt: string;
}
export interface Session { id: string; userId: string; token: string; expiresAt: string; createdAt: string; }

export interface ProviderCredential {
  id: string; userId: string; providerId: string; label: string;
  keyEnc: { iv: string; ct: string } | null;
  endpoint: string | null; defaultModel: string | null;
  status: ProviderStatus; lastCheck: string | null; latencyMs: number | null; lastError: string | null;
  billing: ProviderBilling; extra: Record<string, string>; createdAt: string; updatedAt: string;
}

export interface ModelInfo {
  id: string; providerId: string; userId: string | null;
  name: string; displayName: string; capabilities: Capability[];
  context: number | null; pricingNote: string;
  supports: Record<string, any>; enabled: boolean; source: "catalog" | "discovered"; createdAt: string;
}

export interface StageEvent { stage: string; at: string; honest: boolean; }

export interface Generation {
  id: string; userId: string; jobId: string; type: TaskType; status: JobStatus;
  prompt: string; params: Record<string, any>;
  providerId: string | null; model: string | null; simulated: boolean;
  creditEstimate: number; creditFinal: number | null; assetId: string | null;
  error: string | null; errorCode: string | null; stages: StageEvent[];
  createdAt: string; updatedAt: string;
}
export interface Job {
  id: string; userId: string; generationId: string; status: JobStatus; stage: string;
  requestedCancel: boolean; createdAt: string; startedAt: string | null; finishedAt: string | null; error: string | null;
}
export interface Asset {
  id: string; userId: string; kind: AssetKind; name: string; mime: string; size: number;
  blobId: string | null; url: string | null; width: number | null; height: number | null;
  generationId: string | null; meta: Record<string, any>; createdAt: string;
}
export interface Character {
  id: string; userId: string; name: string; description: string;
  imageAssetId: string | null; videoAssetId: string | null; voice: string | null;
...
export interface PricingRule {
  id: string; taskType: TaskType | "*"; providerId: string; model: string;
  base: number; unit: "per_generation" | "per_second" | "per_megapixel";
  resolutionMult: Record<string, number>; qualityMult: Record<string, number>; note: string;
}
...
export interface ProviderDef {
  id: string; name: string; tagline: string; auth: "apikey" | "token" | "none";
  billing: ProviderBilling; capabilities: Capability[]; docs: string;
}
...
export interface GenRequest {
  type: TaskType; prompt: string; negative?: string;
  width?: number; height?: number; aspect?: string; duration?: number;
  seed?: number; style?: string; camera?: string; language?: string; voice?: string;
  ...
  scenes?: { prompt: string; duration: number; camera: string }[]; script?: string;
  modelHint?: string; signal?: AbortSignal; cancelCheck?: () => boolean;
}
...
export interface MachineProfile {
  ...
  ramMB: number | null;
  gpus: GpuInfo[];
  disk: { totalMB: number | null; freeMB: number | null };
  webgpu: boolean;
  checkedAt: string;
  warnings: string[];
}
export interface OllamaStatus {
  endpoint: string; reachable: boolean; installed: boolean | null;
  version: string | null; latencyMs: number | null;
  models: { name: string; sizeMB: number; vision: boolean; modified: string }[];
  error: string | null;
}
export interface ModelRecommendation {
  name: string; category: "text" | "vision"; sizeMB: number; vramMB: number;
  vision: boolean; tier: "Best" | "Good" | "Experimental"; note: string; installed: boolean;
}
