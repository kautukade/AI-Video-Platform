...
import {
  Activity, AudioLines, Boxes, CheckCircle2, Cpu, ExternalLink, Flower2, Image as ImageIcon, Key, Plug, RefreshCw, Search, Server, ShieldCheck, Star, Trash2, Video, Wifi, WifiOff, XCircle, Zap,
} from "lucide-react";
...
const FREE_DIRECTORY: { provider: string; model: string; name: string; cat: Exclude<Cat, "all">; bestFor: string; freeNote: string }[] = [
  ...
  { provider: "replicate", model: "minimax/video-01", name: "MiniMax Video-01", cat: "video", bestFor: "Talking & product video", freeNote: "Free daily" },
  { provider: "replicate", model: "tencent/hunyuan-video", name: "HunyuanVideo", cat: "video", bestFor: "Consistent, strong quality", freeNote: "Free daily" },
  { provider: "replicate", model: "bytedance/omni-human-1.5", name: "OmniHuman 1.5", cat: "video", bestFor: "Character lip-sync video", freeNote: "Free daily" },
  { provider: "luma", model: "photon-1", name: "Luma Photon", cat: "video", bestFor: "Cinematic shots & camera moves", freeNote: "~30 free / month" },
  ...
  { provider: "replicate", model: "black-forest-labs/flux-schnell", name: "FLUX (Replicate)", cat: "image", bestFor: "Open image models", freeNote: "Free daily" },
  ...
  { provider: "pollinations", model: "openai", name: "GPT (Pollinations)", cat: "text", bestFor: "No-key text generation", freeNote: "Free · no key" },
  { provider: "google", model: "gemini-2.5-flash", name: "Gemini 2.5 Flash", cat: "text", bestFor: "Fast + smart, big context", freeNote: "Free tier" },
  ...
  { provider: "huggingface", model: "microsoft/speecht5_tts", name: "SpeechT5 TTS", cat: "audio", bestFor: "Narration voice for characters", freeNote: "Free tier" },
];
...
function ProviderGlyph({ id }: { id: string }) {
  const map: Record<string, any> = { openrouter: Plug, huggingface: Boxes, pollinations: Flower2, ollama: Server, custom: Cpu, simulator: Zap, google: Key, replicate: Boxes, nvidia: Cpu, together: Plug, groq: Zap, cerebras: Zap, deepseek: Search, mistral: Plug, luma: Video };
  const I = map[id] ?? Plug;
  return <I size={19} />;
}
...
export function ProvidersPage() {
  ...
  const registry = useMemo(() => {
    ...
    try { if (!api.platformMode().simulatorEnabled) return all.filter((p) => p.id !== "simulator"); } catch { /* boot pending */ }
    return all;
  }, []);
  ...
  const providersForCat = useMemo(
    () => registry.filter((p) => cat === "all" || p.capabilities.some((c) => catCap[cat as Exclude<Cat, "all">]?.includes(c))),
    [registry, cat],
  );
  const modelsForCat = useMemo(() => {
    let simOn = false;
    try { simOn = api.platformMode().simulatorEnabled; } catch { /* boot pending */ }
    return FREE_DIRECTORY.filter((m) => (cat === "all" || m.cat === cat) && (simOn || m.provider !== "simulator"));
  }, [cat]);

  return (
    <div className="relative">
      <div className="pointer-events-none absolute -top-10 right-0 h-64 w-64 rounded-full bg-jade-500/6 blur-3xl" />
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-[13.5px] text-ink-400">
            Har free AI engine, category-wise. <strong className="text-ink-200">Login with provider</strong> karo — API key dalte hi system
            <strong className="text-ink-200"> best free model automatic select</strong> kar leta hai.
          </p>
        </div>
        <Tag tone="jade"><ShieldCheck size={12} /> keys AES-GCM encrypted · auto free-model routing</Tag>
      </div>
        ...
        {CATEGORIES.map((c) => {
          ...
          return (
            <button key={c.id} onClick={() => setCat(c.id)}
              className={cn("group flex items-center gap-2.5 rounded-[12px] border px-4 py-2.5 transition-all",
                active ? "border-solar-500/60 bg-solar-400/12 text-solar-300 shadow-[0_4px_20px_-6px_rgba(255,193,77,0.4)]" : "border-ink-700 bg-ink-850/60 text-ink-300 hover:border-ink-500 hover:text-ink-100")}>
              <c.icon size={16} className={active ? "text-solar-400" : "text-ink-400 group-hover:text-ink-200"} />
          ...
          return (
            <div key={p.id} className={cn("panel relative flex flex-col overflow-hidden p-5 transition-all hover:-translate-y-0.5 hover:border-ink-500", connected && "border-jade-500/30")}>
              ...
              <div className="flex items-start justify-between gap-2">
                <div className={cn("flex h-11 w-11 items-center justify-center rounded-xl border", connected ? "border-jade-500/40 bg-jade-500/10 text-jade-300" : "border-ink-600 bg-ink-800 text-ink-300")}>
                ...
                <div className="flex flex-col items-end gap-1.5">
                  ...
                  {connected ? (
                    <Tag tone={conn!.status === "connected" ? "jade" : "coral"}>
                      {conn!.status === "connected" ? <><Wifi size={11} /> connected</> : <><WifiOff size={11} /> error</>}
                    </Tag>
                  ) : noKey ? (
                    <Tag tone="jade"><Zap size={11} /> no login needed</Tag>
                  ) : (
              ...
              {featured.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {featured.map((m) => <Tag key={m.model} tone="ink" className="font-mono">{m.name}</Tag>)}
                </div>
              )}
              <div className="mt-3 flex flex-wrap gap-1.5">
                {p.capabilities.map((c) => <Tag key={c} tone={CAP_TONES[c] ?? "ink"}>{c}</Tag>)}
              </div>
              <div className="mt-4 flex items-center gap-2 border-t border-ink-700 pt-4">
                {connected ? (
                  <>
                    <Button size="sm" variant="jade" onClick={async () => {
                      try { const r = await api.testConnection(conn!.id); toast("success", `${p.name} healthy`, r.message); bump(); }
                      catch (e) { toast("error", `${p.name} test failed`, friendlyError(e).message); bump(); }
                    }} icon={<Activity size={13} />}>Test</Button>
                    <Button size="sm" variant="outline" onClick={() => setModal(p)}>Manage</Button>
                  ...
                  <>
                    <Button size="sm" onClick={() => setModal(p)} icon={noKey ? <Zap size={13} /> : <Key size={13} />}>
                      {noKey ? "Connect" : `Login ${p.name.split(" ")[0]}`}
                    </Button>
                    <a href={p.docs} target="_blank" rel="noreferrer" className="ml-auto flex items-center gap-1 text-[11.5px] font-semibold text-ink-400 hover:text-solar-300">
                      {noKey ? "docs" : "free key"} <ExternalLink size={11} />
                    </a>
                  </>
                )}
              </div>
      ...
      <div className="mt-10">
        <div className="mb-4 flex items-end justify-between">
          ...
          <span className="font-mono text-[11px] text-ink-500">verified catalog · updated per provider docs</span>
        </div>
                ...
                return (
                  <tr key={m.provider + m.model} className="border-b border-ink-800 transition-colors last:border-0 hover:bg-ink-800/50">
                    <td className="px-4 py-3">
                      <div className="font-bold text-ink-100">{m.name}</div>
                      <div className="font-mono text-[10.5px] text-ink-500">{m.model}</div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {conn ? (
                        <Tag tone="jade"><CheckCircle2 size={11} /> ready</Tag>
                      ) : (
                        <Button size="sm" variant="outline" onClick={() => def && setModal(def)}>Login & Use</Button>
                      )}
                    </td>
                  </tr>
                );
      ...
      <ConfirmModal open={!!confirmDisc} onClose={() => setConfirmDisc(null)} title={`Disconnect ${confirmDisc?.label}?`}
        body="Saved key encrypted vault se delete ho jayegi. Models is provider ke liye unavailable ho jayenge." confirmLabel="Disconnect"
    ...
    try {
      if (def.id === "ollama") {
        ...
        const r = await api.connectProvider({
          providerId: def.id, apiKey: apiKey || undefined,
          endpoint: def.id === "custom" ? endpoint : undefined,
          defaultModel: defaultModel || undefined,
          extra: def.id === "custom" && imgEndpoint ? { imageEndpoint: imgEndpoint } : {},
        });
      ...
      setResult({ ok: false, message: friendlyError(e).message });
    ...
    <Modal open onClose={onClose} title={<span className="flex items-center gap-2"><ProviderGlyph id={def.id} /> Login {def.name}</span>}
      footer={def.id === "ollama" ? (
        <>
          ...
          <Button variant="outline" loading={validating} icon={<Activity size={13} />} onClick={validate}>Detect Ollama</Button>
          <Button loading={busy} onClick={connectOllama} disabled={result ? !result.ok : false}>Connect Ollama</Button>
        </>
      ) : (
        <>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          {needsKey && <Button variant="outline" loading={validating} icon={<Activity size={13} />} onClick={validate} disabled={!apiKey.trim() && !existing}>Validate</Button>}
          <Button loading={validating || busy} onClick={validate} icon={<Key size={13} />}>{existing ? "Save" : "Login & Connect"}</Button>
        </>
      )}>
      <div className="space-y-4">
        {needsKey && (
          <a href={def.docs} target="_blank" rel="noreferrer"
            className="flex items-center justify-between rounded-[10px] border border-solar-500/35 bg-solar-400/8 px-4 py-3 text-[12.5px] font-bold text-solar-300 transition-colors hover:bg-solar-400/15">
        ...
        {needsKey && (
          <Field label={def.auth === "token" ? "API Token" : "API Key"} hint={existing ? "leave blank to keep current key" : undefined}>
            <Input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} autoComplete="off"
              placeholder={def.id === "openrouter" ? "sk-or-v1-…" : def.id === "huggingface" ? "hf_…" : "API key…"} />
            <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-ink-500"><Key size={11} /> AES-GCM encrypted · sirf masked hint dikhega · kabhi log nahi hota</div>
          </Field>
        )}
        {(def.id === "ollama" || def.id === "custom") && (
          <Field label={def.id === "ollama" ? "Ollama endpoint" : "Base URL"} hint={def.id === "ollama" ? "localhost, LAN IP ya hostname" : "must expose /models & /chat/completions"}>
        ...
        {def.id === "custom" && (
          <Field label="Image endpoint (optional)" hint="defaults to {base}/images/generations">
            <Input value={imgEndpoint} onChange={(e) => setImgEndpoint(e.target.value)} placeholder="https://…/v1/images/generations" />
          </Field>
        )}
        <Field label="Default model (optional)" hint="blank → system auto-selects best free model">
        ...
        {def.id === "ollama" && (
          <div className="rounded-[10px] border border-ink-700 bg-ink-850 p-4">
            ...
            {ollamaModels === null ? (
              <p className="mt-2 text-[12px] text-ink-500"><span className="font-mono text-solar-300">Detect Ollama</span> dabao — <span className="font-mono">/api/tags</span> se models list honge.</p>
            ) : ollamaModels.length === 0 ? (
              <p className="mt-2 text-[12px] text-ink-500">Ollama reachable hai par koi model nahi. Pehle pull karo: <span className="font-mono text-solar-300">ollama pull llama3.2</span></p>
            ) : (
            ...
            {result && !result.ok && (
              <div className="mt-3 border-t border-ink-700 pt-3">
                ...
                <ol className="mt-1.5 list-decimal space-y-1 pl-4 text-[12px] text-ink-400">
                  ...
                  <li>Endpoint URL aur port check karo (default 11434)</li>
                  <li>Browser-direct ke liye CORS: <span className="font-mono text-ink-200">OLLAMA_ORIGINS=* ollama serve</span></li>
                  <li>Ya local bridge chalao: <span className="font-mono text-ink-200">node local-bridge.mjs</span> (CORS-free)</li>
                </ol>
              </div>
            )}
            <div className="mt-3 border-t border-ink-700 pt-3 text-[11.5px] leading-relaxed text-ink-400">
              <span className="font-bold text-ink-200">Cloud models (ollama.com):</span> terminal me <span className="font-mono text-solar-300">ollama signin</span> chalao — cloud models bhi detect honge.{" "}
        ...
        {def.id === "pollinations" && <InfoNote tone="jade"><strong>No login, no key.</strong> Pollinations free FLUX images aur GPT text publicly serve karta hai — Connect dabao aur routing turant shuru.</InfoNote>}
        {def.id === "huggingface" && <InfoNote>Free tier me <strong>image, video, text & audio</strong> sab hai. Video models cold-start pe slow ho sakte hain — honest <strong>MODEL_LOADING</strong> status dikhta hai.</InfoNote>}
        {def.id === "openrouter" && <InfoNote>Free OpenRouter account → key. Studio <strong>":free" model variants automatic prefer</strong> karta hai (Llama 3.3, DeepSeek). Paid models sirf aapki explicit permission pe.</InfoNote>}
        {def.id === "replicate" && <InfoNote>Free account pe <strong>daily free predictions</strong> milte hain — LTX, Wan 2.1, MiniMax, HunyuanVideo aur OmniHuman character video ke liye best.</InfoNote>}
        {def.id === "nvidia" && <InfoNote>NIM free tier me <strong>1000 credits</strong> milte hain — LTX Video aur Llama models ke liye. Key build.nvidia.com se.</InfoNote>}
        {def.id === "luma" && <InfoNote>Dream Machine ka <strong>free monthly quota</strong> — cinematic text-to-video aur image-to-video (character keyframes).</InfoNote>}
      </div>
    </Modal>
  );
}
...
export function ModelsPage() {
  ...
  const models = useMemo(() => {
    try { return api.listModels({ capability: cap === "all" ? undefined : cap }); } catch { return []; }
  }, [tick, cap]);
  ...
  return (
    <div>
      <div className="mb-5">
        ...
        <p className="mt-1 text-[13px] text-ink-400">Connected providers ke saare models — capability badges aur pricing ke saath. Sirf connected providers ke models dikhte hain.</p>
      </div>
      ...
      <div className="stagger mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {filtered.map((m) => (
          <div key={m.id} className="panel-flat p-4 transition-colors hover:border-ink-500">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="text-[13.5px] font-bold text-ink-50">{m.displayName}</div>
                <div className="mt-0.5 font-mono text-[10.5px] text-ink-500">{m.name}</div>
              </div>
